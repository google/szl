...
static int
fmtzdotpad(State *f, int n, int pt)
{
	char *t = NULL;  /* initialize to avoid compiler warning */
	char *s = NULL;  /* initialize to avoid compiler warning */
	const char *p;
	int i, w;
	Rune *rt, *rs, r;
...
__efgfmt(State *fmt)
{
	...
	case 'g':
		/*
		 * decide on 'e' or 'f' style conversion
		 */
		if(e >= -4 && e <= prec) {
			c1 = -e;
			c4 = prec - e;
	...
	if((fl & (FmtWidth|FmtLeft)) == FmtWidth && n < fmt->width){
		if(fl & FmtZero){
			...
			if(__rfmtpad(fmt, fmt->width - n) < 0){
				return -1;
			}
		}else{
	...
	d = 0;
	if(c4 >= 0 && c4 < c2){
		if(__fmtcpy(fmt, s1, c4, c4) < 0 || fmtstrcpy(fmt, fmt->decimal) < 0)
			return -1;
		d = c4;
		c2 -= c4;
		c4 = -1;
	 ...
	 */
	if((fl & FmtSharp) && c4 == 0 && fmtstrcpy(fmt, fmt->decimal) < 0){
		return -1;
	}
	if(chr == 'e' || chr == 'g') {
		d = 0;
